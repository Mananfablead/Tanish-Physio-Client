  ...
  <div className="flex flex-col lg:flex-row gap-8 mb-8">
    <div className="lg:w-1/2">
      <div className="flex items-start gap-4 mb-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-slate-900 mb-2">{service.details.title}</h1>
          <div className="flex flex-wrap gap-3 mb-3">
            <Badge variant="secondary" className="px-4 py-2 rounded-full text-sm font-medium bg-primary/10 text-primary border-primary/20">
              {service.details.sessionDuration}
            </Badge>
            <Badge variant="secondary" className="px-4 py-2 rounded-full text-sm font-medium bg-primary/10 text-primary border-primary/20 flex items-center gap-1">
              <IndianRupee className="h-4 w-4" />
              {service.details.priceRange.replace('₹', '')}
            </Badge>
          </div>
        </div>
      </div>
      <p className="text-lg text-slate-600 mb-6">{service.details.description}</p>
      <p className="text-slate-600 mb-6">{service.details.detailedDescription}</p>
    </div>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="relative w-full max-w-4xl bg-white rounded-2xl p-4">
            ...
            <div className="aspect-video bg-slate-100 rounded-xl flex items-center justify-center">
              <p className="text-slate-500">Video Player: {service.media.videoUrl}</p>
            </div>
          </div>
        </div>
      )}
...
const ServiceOverview = ({ service }: { service: ExtendedService }) => (
  <div className="bg-slate-50 rounded-2xl p-6 border border-slate-200 mb-8">
    <h3 className="text-xl font-bold text-slate-900 mb-4">Quick Overview</h3>
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
      <div>
        <p className="text-sm font-medium text-slate-500 uppercase tracking-wider mb-1">Duration</p>
        <p className="font-bold text-slate-900">{service.details.sessionDuration}</p>
      </div>
      <div>
        <p className="text-sm font-medium text-slate-500 uppercase tracking-wider mb-1">Results Timeline</p>
        <p className="font-bold text-slate-900">{service.details.resultsTimeline}</p>
      </div>
      <div>
        <p className="text-sm font-medium text-slate-500 uppercase tracking-wider mb-1">Price Range</p>
...
const ServiceSidebar = ({ service, navigate }: { service: ExtendedService; navigate: any }) => (
  <div className="space-y-6 sticky top-24">
    <div className="bg-slate-50 rounded-2xl p-6 border border-slate-200">
      ...
      <div className="space-y-4 mb-4">
        <div className="flex justify-between">
          ...
          <span className="font-medium text-slate-900">{service.details.sessionDuration}</span>
        </div>
        <div className="flex justify-between">
        ...
        <div className="flex justify-between">
          ...
          <span className="font-medium text-slate-900">{service.details.resultsTimeline}</span>
        </div>
        {service.details.prerequisites && (
          <div className="flex justify-between">
            <span className="text-slate-600">Prerequisites:</span>
            <span className="font-medium text-slate-900">{service.details.prerequisites}</span>
          </div>
        )}
      </div>
      ...
      <div className="mt-4 space-y-3 pt-4 border-t border-slate-200">
        ...
        <div className="flex items-start gap-2">
          <CheckCircle className="h-5 w-5 text-primary mt-0.5 flex-shrink-0" />
          <span className="text-sm text-slate-600">Evidence-based treatment</span>
        </div>
        <div className="flex items-start gap-2">
          <CheckCircle className="h-5 w-5 text-primary mt-0.5 flex-shrink-0" />
    ...
    <div className="bg-slate-50 rounded-2xl p-6 border border-slate-200">
      ...
      <p className="text-slate-600 italic mb-3">"The therapy sessions helped me recover faster than I expected. The therapist was very professional and caring."</p>
      <p className="text-sm text-slate-500">- Sarah J., Orthopedic Physiotherapy</p>
    </div>
  </div>
);

// Collapsible List Component
const CollapsibleList = ({ title, items, icon }: { title: string; items: string[]; icon: React.ReactNode }) => {
...
export default function ServiceDetailPage() {
  ...
  const service = useMemo(() => services.find(s => s.id === parseInt(serviceId || "")), [serviceId]);
        ...
        <div className="container">
              ...
              <div className="lg:col-span-2 space-y-8">
                ...
                <div className="border-t border-slate-200 pt-8">
                  <CollapsibleList 
                    ...
                    icon={<div className="h-2 w-2 rounded-full bg-primary flex-shrink-0 mt-2" />} 
                  />
                </div>
                ...
                <div className="border-t border-slate-200 pt-8">
                  <CollapsibleList 
                    ...
                    icon={<CheckCircle className="h-5 w-5 text-primary flex-shrink-0 mt-0.5" />} 
                  />
                </div>
              </div>
          ...
          <div className="fixed bottom-6 left-6 right-6 lg:hidden">
            <div className="flex items-center justify-between bg-white rounded-xl p-3 mb-2 shadow-lg border">
              <div>
                <p className="text-sm font-medium text-slate-900">{service.details.priceRange.replace('₹', '')}</p>
                <p className="text-xs text-slate-500">{service.details.sessionDuration}</p>
              </div>
